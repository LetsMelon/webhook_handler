...
use shared::docker::{ContainerState, ContainerSummary};
...
impl DockerConnection {
    pub fn new() -> Result<Self> {
        let raw_connection = unsafe { crate::raw::docker_connection_new() };
    ...
    pub fn ping(&self, timeout: Option<u32>) -> Result<PingResult> {
        ...
        let ping_result = unsafe { crate::raw::docker_ping(self.inner, timeout) };
    ...
    pub fn get_container_by_name(&self, name: &str) -> Result<Option<ContainerSummary>> {
        ...
        let container_name_result = unsafe {
            crate::raw::docker_get_container_by_name(
                self.inner,
                name_c_str.as_ptr(),
                &mut containers_ptr,
                &mut containers_len,
        ...
        let packet = postcard::from_bytes(data_raw)?;
    ...
    pub fn stop_container(&self, container_id: &str, timeout: Option<u32>) -> Result<()> {
        ...
        let result = unsafe {
            crate::raw::docker_stop_container(self.inner, container_id_c_str.as_ptr(), timeout)
        };
    ...
    pub fn remove_container(&self, container_name: &str, force: bool) -> Result<()> {
        ...
        let result =
            unsafe { crate::raw::docker_remove_container(self.inner, name_c_str.as_ptr(), force) };
    ...
    pub fn create_container(&self, container_name: &str, exposed_ports: &[&str]) -> Result<()> {
        ...
        let result = unsafe {
            crate::raw::docker_create_container(
                self.inner,
    ...
    pub fn start_container(&self, container_name: &str) -> Result<()> {
        ...
        let result = unsafe {
            crate::raw::docker_start_container(self.inner, container_name_c_string.as_ptr())
        };
...
impl Drop for DockerConnection {
    fn drop(&mut self) {
        unsafe { crate::raw::docker_connection_free(self.inner) }
    }
}
